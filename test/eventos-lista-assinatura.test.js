...
test('sem filtro de status: traz todos os inscritos, ordenados', async () => {
  ...
  await rotas['/eventos/:id/lista-assinatura']({ params: { id: '5' }, query: {} }, resFake());
...
test('status=confirmado filtra pelo status confirmado', async () => {
  ...
  await rotas['/eventos/:id/lista-assinatura']({ params: { id: '5' }, query: { status: 'confirmado' } }, resFake());
  const q = queries.find(q => /evento_inscricoes/.test(q.sql));
...
test('status=pendente filtra pelo status pendente e rotula "Pendientes" no cabeçalho', async () => {
  ...
  await rotas['/eventos/:id/lista-assinatura']({ params: { id: '5' }, query: { status: 'pendente' } }, resFake());
...
test('valor de status desconhecido é ignorado (não filtra, não quebra)', async () => {
  const { queries, rotas } = montar({ evento: eventoBase, inscricoes: [] });
  await rotas['/eventos/:id/lista-assinatura']({ params: { id: '5' }, query: { status: 'lixo' } }, resFake());
  const q = queries.find(q => /evento_inscricoes/.test(q.sql));
...
test('evento inexistente devolve 404, sem tentar gerar nada', async () => {
  const { rotas, chamadaGerar } = montar({ evento: null });
  const res = resFake();
  await rotas['/eventos/:id/lista-assinatura']({ params: { id: '999' }, query: {} }, res);
  assert.strictEqual(res._status, 404);
  assert.strictEqual(chamadaGerar(), null);
});

test('o HTML final vem com o script de auto-impressão injetado', async () => {
  const { rotas } = montar({ evento: eventoBase, inscricoes: [] });
  const res = resFake();
  await rotas['/eventos/:id/lista-assinatura']({ params: { id: '5' }, query: {} }, res);
