...
async function enviarWhatsApp(numero, mensagem) {
  ...
  if (!token || !instanceId) {
    console.warn('W-API: ZAPAPI_TOKEN ou ZAPAPI_INSTANCE não configurado');
    return { ok: false };
  }
 
  const fone = formatarNumero(numero);
  console.log(`W-API enviando para ${fone} instancia ${instanceId} token ${token.substring(0,8)}...`);
 
  try {
    const { data, status } = await axios.post(
      `https://api.w-api.app/v1/message/send-text?instanceId=${instanceId}`,
      { phone: fone, message: mensagem, instanceId, delayMessage: 1 },
      {
        headers: {
    ...
    const d = JSON.stringify(err.response?.data || err.message).substring(0, 300);
