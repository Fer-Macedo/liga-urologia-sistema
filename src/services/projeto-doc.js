...
const runG = (t, o = {}) => new TextRun({ text: t || '', bold: !!o.b, font: FONTE, size: o.sz || 24, ...(o.color ? { color: o.color } : {}) });
const runS = (t, o = {}) => new TextRun({ text: t || '', bold: o.b !== false, font: FONTE, size: o.sz || 24 });
...
const sec = (n, t) => new Paragraph({
  ...
  children: [ runG(romano(n) + '.', { b: true }), new TextRun({ text: '\t', font: FONTE, size: 24 }), runG(t, { b: true }) ]
});
...
function tabelaCronograma(p, totalH) {
  ...
  // cabeçalho AZUL com texto branco (head=true)
  const cell = (txt, w, bold, span, head) => new TableCell({
    borders: bds, width: { size: w, type: WidthType.DXA }, margins: mg,
    columnSpan: span,
    ...(head ? { shading: { fill: '2E74B5', type: ShadingType.CLEAR, color: 'auto' } } : {}),
    children: [par(runG(txt, { b: !!bold, ...(head ? { color: 'FFFFFF' } : {}) }), { line: 240 })]
  });
  ...
  const rows = [
    new TableRow({ tableHeader: true, children: head.map((h, i) => cell(h, cw[i], true, undefined, true)) }),
    ...(p.cronograma || []).map(c => new TableRow({ children: [
      cell(c.atividade || '', cw[0]), cell(c.responsavel || '', cw[1]), cell(fmtD(c.data), cw[2]),
      cell(hm(c.hora_inicio), cw[3]), cell(hm(c.hora_fim), cw[4]), cell(c.horas_total ? c.horas_total + 'h' : '', cw[5])
    ] })),
...
function montarProjeto(p, totalH) {
  ...
  if (ehEns) {
    especifico = [
      ...
      sec(8, 'INSCRIPCIÓN'),
      // Texto corrido conforme o modelo aprovado (mesma redação do gerador timbrado).
      pTxt(require('./projeto-texto').textoInscripcion(p)),
