...
router.get('/certificado/validar/:codigo', async (req, res) => {
  try {
    ...
    const cert = r.rows[0];
    const config = await getConfig();
    if (!cert) return res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Certificado Inválido</title><style>body{font-family:Arial,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;background:#f8fafc}div{text-align:center;padding:40px;background:white;border-radius:16px;box-shadow:0 4px 20px rgba(0,0,0,.08);max-width:440px}</style></head><body><div><div style="font-size:48px;margin-bottom:16px">❌</div><h2 style="color:#dc2626;margin-bottom:8px">Certificado Inválido</h2><p style="color:#6b7280">Este código de verificação não corresponde a nenhum certificado emitido pela ${config.org_nome||'LAURO'}.</p></div></body></html>`);
