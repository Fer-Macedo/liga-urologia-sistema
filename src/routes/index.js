...
router.get('/dashboard', requireAuth, async (req, res) => {
  ...
  const [total, pagos, pendentes, atrasados, recTot, pendTot, atrTot, recentes, aniversariantes] = await Promise.all([
    ...
    query("SELECT *, TO_CHAR(data_nascimento::date,'MM-DD') as aniv FROM membros WHERE ativo=1 AND data_nascimento IS NOT NULL ORDER BY CASE WHEN TO_CHAR(data_nascimento::date,'MM-DD') >= TO_CHAR(NOW(),'MM-DD') THEN TO_CHAR(data_nascimento::date,'MM-DD') ELSE TO_CHAR(data_nascimento::date,'MM-DD') || 'z' END LIMIT 6")
  ]);
